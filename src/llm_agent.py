...
if not groq_api_key and not claude_api_key:
    raise ValueError("Kindly add your groq api key")
...
class RFEBOT:
    ...
    def Agentprompt(petitionContent: str) -> Dict:
        ...
        Highlight any media features, interviews, or eEvidence of judging/peer review activities.

        # Identify Potential Red Flags  
        Carefully evaluate the petition for any signs that could weaken the credibility or strength of the eEvidence provided. Flag any of the following:
        ...
        6. Insufficient Supporting EEvidence  
        ...
        8. Inconsistent or Broad Field of Expertise  
         EEvidence presented across unrelated or loosely connected domains (e.g., AI + graphic design + biology)  
        ...
        {{
            "personal_background": "...",
            "Criterion  :xpert_recommendation_letters": ["...", "..."],
        ...
        """
        prompt2: str = f"""You are a USCIS Officer assigned to critically review an EB-1A petition. Your task is to extract eEvidence supporting each EB-1A criterion from the petition content.
        ...
        (ix) EEvidence of commanding a high salary or significantly high remuneration relative to others in the field.
        ...
        combined_response = {
            ...
            "Criterion  :xpert_recommendation_letters": response1.get(
                "Criterion  :xpert_recommendation_letters", []
            ),
            "Criterion  :Evidence": response2.get("Criterion  :Evidence", {}),
    ...
    def AgentpromptInternationalAward(petitionContent: str) -> Dict:
        ...
        1. Check if the document contains any eEvidence of the beneficiary receiving a **major, internationally recognized award** (e.g., Nobel Prize, Olympic Medal, Pulitzer Prize).
        ...
        6. Under `suggested_supporting_eEvidence`, propose what kind of documentation could strengthen the petitioner's claim that the award qualifies (based on the RFE guidance).
        ...
        "justification": "...",
        "missing_elements": ["..."], 
        "suggested_supporting_eEvidence": ["..."]
        }}
        """
    ...
    def AgenticCriterionPrompt1(petitionContent: str) -> Dict:
        ...
        2. Set `meets_criterion_1` to `true` only if awards have strong eEvidence of being:
        ...
        4. Provide analysis in JSON format with decision reasoning, detected issues, missing elements, and suggested supporting eEvidence.
    ...
    def AgenticCriterionPrompt2(petitionContent: str) -> Dict:
        ...
        3. Carefully evaluate for RFE indicators. If applicable, list problems in `issues_detected`. Common issues include:
         ...
         No eEvidence that reviewers are **recognized experts**.
         No **bylaws or membership criteria** included.
         Only student-level or early-career membership provided.

        4. Use `missing_elements` to highlight gaps in eEvidence. These may include:
        ...
        5. In `suggested_supporting_eEvidence`, recommend helpful documentation, such as:
         ...
         EEvidence that association membership is limited and prestigious.
    ...
    def AgenticCriterionPrompt3(petitionContent: str) -> Dict:
        ...
        6. In `suggested_supporting_eEvidence`, recommend documents the petitioner could include to strengthen this criterion, such as:
         ...
         EEvidence of independent reporting (not self-promotional)
    ...
    def AgenticCriterionPrompt4(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 4**: "Criterion  :Evidence of the beneficiary's participation, either individually or on a panel, as a judge of the work of others in the same or an allied field of specialization for which classification is sought."
        ...
        4. Include in `issues` if:
         ...
         There is no eEvidence of actual judging (e.g. invitation without confirmation).
        ...
        6. Recommend `suggested_supporting_eEvidence` such as:
        ...
        "judging_instances": [
            {{
            "Criterion  :vent_name_or_platform": "...",
            "role": "...",
            "date_or_period": "...",
    ...
    def AgenticCriterionPrompt5(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 5**: "Criterion  :Evidence of the beneficiary's original scientific, scholarly, artistic, athletic, or business-related contributions of major significance in the field."
        ...
        4. In `eEvidence_summary`, include what was submitted (e.g., journal articles, patents, letters from experts, media coverage, usage contracts, licenses).
        ...
        9. Recommend `suggested_supporting_eEvidence` such as:
         Expert letters describing how the work is both original and significant
         EEvidence of use in industry (contracts, deployment, citations, partnerships)
    ...
    def AgenticCriterionPrompt6(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 6**: "Criterion  :Evidence of the beneficiary's authorship of scholarly articles in the field, in professional or major trade publications or other major media."
        ...
        4. In `eEvidence_summary`, include any submitted materials such as:
        ...
        7. Recommend `suggested_supporting_eEvidence` such as:
    ...
    def AgenticCriterionPrompt7(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 7**:  **"Criterion  :Evidence of the display of the beneficiary's work in the field at artistic exhibitions or showcases."**
        ...
        4. Use `eEvidence_summary` to list submitted materials such as:
        ...
        5. Use `issues` to flag any problems, such as:
         ...
         Lack of eEvidence linking displayed work to the petitioner
        ...
        7. Use `suggested_supporting_eEvidence` to list ways to strengthen the petition, such as:
    ...
    def AgenticCriterionPrompt8(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 8**:  **"Criterion  :Evidence that the beneficiary has performed in a leading or critical role for organizations or establishments that have a distinguished reputation."**
        ...
        2. For each organization:
         ...
         `is_leading_or_critical`: `true` if the eEvidence shows the petitioner's performance significantly influenced the organization
         ...
         `reputation_eEvidence`: List sources (e.g., media recognition, awards, rankings, partnerships, market share, historical significance, etc.)
         ...
         `issues`: Highlight any problems, e.g., unclear responsibilities, weak reputation eEvidence, unclear link between role and outcome

        3. `meets_criterion_8: true` only if **at least one** organization qualifies under:
         Role is well-eEvidenced as leading or critical
         Organization is demonstrated to be of **distinguished reputation**

        4. Use `missing_elements` to list gaps in the eEvidence, such as:
        ...
        5. Use `suggested_supporting_eEvidence` to recommend how to improve, such as:
    ...
    def AgenticCriterionPrompt9(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 9**:  **"Criterion  :Evidence that the beneficiary has commanded a high salary or other significantly high remuneration for services, in relation to others in the field."**
        ...
        2. For each salary/remuneration claim:
         `year`: The tax year or employment period the eEvidence covers
         `salary_amount`: Total salary/remuneration stated (with currency)
         `source`: Type of eEvidence (e.g., W-2, 1099, foreign tax form, salary letter, contract)
         `comparison_eEvidence_provided`: `true` if the petition includes documents showing how this salary compares to others in the same field/region
         `is_significantly_high`: `true` if supported eEvidence clearly shows the salary is above industry norms
         `supporting_documents`: List submitted materials such as:
        ...
        "salary_records": [
            ...
            "salary_amount": "...",
            "source": "...",
            "comparison_eEvidence_provided": true | false,
    ...
    def AgenticCriterionPrompt10(petitionContent: str) -> Dict:
        prompt: str = f"""You are a USCIS Officer reviewing an EB-1A petition. Your task is to evaluate whether the petitioner satisfies **Criterion 10**:  **"Criterion  :Evidence of commercial successes in the performing arts, as shown by box office receipts or record, cassette, compact disk, or video sales."**
        ...
        1. Determine whether the beneficiary has demonstrated **commercial success in the performing arts**.
         Commercial success must be eEvidenced by **volume of sales** or **box office receipts**
        ...
        2. For each production or work claimed:
         ...
         `supporting_documents`: List submitted materials such as:
             ...
             Third-party eEvidence showing comparative success
         ...
         **Commercially successful** based on volume of sales/receipts
         Supported with concrete, quantifiable eEvidence
        ...
        5. Use `suggested_supporting_eEvidence` to recommend improvements, such as:
